...
public class Model {
	...
	public void plantExplosions() {
		...
		Iterator<Plant> i = plants.listIterator();
		while(i.hasNext()) {
			Plant curr = i.next();
	...
	public boolean isSpaceAvailable(int r, int c) {
		for(Plant p: plants) {
			if(p.getRow() == r && p.getCol() == c && p.isAlive()) {
				return false;
			}
		}
		for(Fish f : fish) {
			if(f.getRow() == r && f.getCol() == c && f.isAlive()) {
				return false;	
			}
		}
	...
	public Model(Model other) {
		...
		plants = new ArrayList<Plant>();
		for(Plant x : other.plants) {
			plants.add(new Plant(x));
		}
...
//		}
	...
	private boolean canMove(Fish f) {
		if(f.getDirection() == Fish.UP && landscape[f.getRow() - 1][f.getCol()] ==  ROCK) {
			return false;
		}
		else if(f.getDirection() == Fish.DOWN && landscape[f.getRow() + 1][f.getCol()] ==  ROCK) {
			return false;
		}
		else if(f.getDirection() == Fish.LEFT && landscape[f.getRow()][f.getCol() - 1] ==  ROCK) {
			return false;
		}
		else if(f.getDirection() == Fish.RIGHT && landscape[f.getRow()][f.getCol() + 1] ==  ROCK) {
			return false;
		}
		return true;
	}
...
//					if(curr.getDirection() != Fish.DOWN && curr.getDirection() != Fish.UP && curr.getDirection() != Fish.LEFT && curr.getDirection() != Fish.RIGHT) {
...
//		}
		for(Fish x : fish) {
			if(x.isAlive()) {
				if(fishIsSurroundedByRocks(x) == false) {
					if(x.getDirection() != Fish.DOWN && x.getDirection() != Fish.UP && x.getDirection() != Fish.LEFT &&
							x.getDirection() != Fish.RIGHT) {
						throw new IllegalFishDirectionException(x.getDirection());
					}
					while(canMove(x) == false) {
...
//						Plant x = p.next();
//						if(x.isAlive()) {
//							if(x.getRow() == curr.getRow() && x.getCol() == curr.getCol()) {
//								fishEatPlant(curr, x);
//							}
//						}
//					}
//					Iterator<Fish> a = fish.iterator();
//					while(a.hasNext()) {
//						Fish b = a.next();
//						if((b.isAlive()) && (b != curr)) {
//							if(b.getRow() == curr.getRow() && b.getCol() == curr.getCol()) {
//								curr.fight(b);
//							}
//						}
//					}
...
//		}
		for(Fish x : fish) {
			if(x.isAlive()) {
				if(fishIsSurroundedByRocks(x) == false) {
					...
					for(Plant p : plants) {
						if(p.isAlive()) {
							if(p.getRow() == x.getRow() && p.getCol() == x.getCol()) {
								fishEatPlant(x, p);
							}
						}
					}
					for(Fish f: fish) {
						if(f.isAlive() && f!= x) {
							if(f.getRow() == x.getRow() && f.getCol() == x.getCol()) {
								x.fight(f);
							}
						}
					}
	...
	public void addPlant(Plant p) {
		...
		for(Plant x : plants) {
			if(x.isAlive()) {
				if(x.getRow() == p.getRow() && x.getCol() == p.getCol() && x != p) {
...
//	}
//	for(Fish j : fish) {
//		if((j.isAlive()) && (j!=f) && j.getRow() == row && j.getCol() == col) {
//			f.fight(j);
//		}
//	}
//}
